...
`pnpm --filter @ot/sdk generate` reads `apps/web/lib/openapi.ts`, writes `openapi.json` and `src/schema.d.ts`. Commit both. `pnpm --filter @ot/sdk test` checks the client against a mocked fetch and the signature helper.
