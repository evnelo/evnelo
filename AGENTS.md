...
- Marketing hero: scene geometry and responsive card styling live together in `components/marketing/hero-scene.tsx` and `hero.css`; use the shared 1000 × 620 canvas on desktop, a three-then-two grid at 641–1023px, and stacked checkpoints on phones. The public header is 112px below 768px and 64px above; keep the Discover filter sticky offset aligned. Desktop scene placement follows the copy height to preserve button clearance; do not restore a fixed aspect ratio on the outer hero box. See `docs/DESIGN.md` for its reference-specific ribbon exception and asset attribution.
