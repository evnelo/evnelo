...
- Marketing hero: scene geometry and responsive card styling live together in `components/marketing/hero-scene.tsx` and `hero.css`; use the shared 1000 × 620 canvas on desktop and stacked checkpoints on phones. See `docs/DESIGN.md` for its reference-specific ribbon exception and asset attribution.
