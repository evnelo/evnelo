...
- Discount codes (decided 2026-09-10): event-wide codes applied to the whole order (per-ticket-type restrictions are a follow-up); the discount is taken before tax and the platform fee; a use is consumed at order creation and returned if the order never pays, so a code's budget is never burned by abandoned checkouts.
- Waitlist (decided 2026-09-10): organizer-driven promotion, not automatic. A promotion holds one seat for 24 hours and emails a claim link bound to the entry's email; automatic promotion on cancellation is a follow-up once organizers ask for it. Event capacity is enforced at checkout (it was only displayed before) and waitlist holds count against it.
- Private events (decided 2026-09-10): access is a cookie set by an invitation link rather than a query parameter carried through registration, so shared screenshots and referrers do not leak tokens; email-bound invites are enforced at checkout, shareable links carry a use budget; organization members always see their own events. No sign-in is required to use an invite.
- Check-in (decided 2026-09-10): browser-based, no native app. Offline support is a device-side manifest of hashed ticket tokens (validates scans and marks them locally) plus a replay queue; a scan that cannot be verified offline is refused rather than accepted on trust. QR payload stays the `/t/{token}` URL so paper, screen and wallet passes scan the same way. Check-in staff never see the dashboard: the `checkin` role lands on `/dashboard/checkin`.
