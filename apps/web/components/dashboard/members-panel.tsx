        ...
        <TBody>
          {members.map((m) => (
            <TR key={m.userId}>
              ...
              <TD className="text-muted-foreground">{new Date(m.since).toLocaleDateString()}</TD>
              {canManage && <TD className="text-right">{m.userId !== currentUserId && <Button size="sm" variant="ghost" disabled={pending} onClick={() => { if (window.confirm(`Remove ${m.email} from the organization?`)) run(() => removeMemberAction(m.userId)); }}>Remove</Button>}</TD>}
            </TR>
          ))}
          {invites.map((i) => (
            <TR key={i.id} className="text-muted-foreground">
              ...
              <TD>—</TD>
              {canManage && <TD className="text-right"><Button size="sm" variant="ghost" disabled={pending} onClick={() => run(() => revokeInviteAction(i.id))}>Revoke</Button></TD>}
            </TR>
          ))}
        </TBody>
      </Table>
