...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: event.name, images: event.coverImageUrl ? [event.coverImageUrl] : [], siteName: org.name, type: "website" },
  };
}
...
export default async function EventPage({ params }: Params) {
  ...
  const jsonLd = {
    ...
    image: event.coverImageUrl, url: `${env.APP_URL}/e/${event.slug}`, organizer: { "@type": "Organization", name: org.name },
    ...
    location: event.locationType === "online" ? { "@type": "VirtualLocation", url: `${env.APP_URL}/e/${event.slug}` } : { "@type": "Place", name: event.venueName, address: event.address },
        ...
        <div className="min-w-0">
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <div className="text-sm">
              <div>Hosted by <a href={`/o/${org.slug}`} className="font-medium hover:underline">{org.name}</a></div>
