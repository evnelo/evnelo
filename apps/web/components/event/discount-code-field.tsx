  ...
  return (
    <div className="animate-rise">
      ...
      <div className="mt-1.5 flex gap-2">
        <Input id="discount-code" value={code} onChange={(e) => { setCode(e.target.value.toUpperCase()); if (applied) onChange(null); }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void apply(); } }} placeholder={t("discount.placeholder")} autoCapitalize="characters" spellCheck={false} className="h-11 font-mono uppercase tracking-wider" />
      ...
      {applied && !error && <p className="mt-1 text-xs text-muted-foreground">{t(applied.totalMinor === 0 ? "discount.offFree" : "discount.off", { code: applied.code, amount: formatMoney(applied.discountMinor, applied.currency, locale) })}</p>}
    </div>
  );
}
