...
- Event cover/logo uploads are authenticated, origin-checked (fail closed), rate limited per user, capped at 5 MB, decoded with `sharp` under a 12-megapixel budget and at most two concurrent decodes, verified by decoded format (not the declared MIME) to be JPEG/PNG/WebP, re-encoded as WebP, and stored under `UPLOAD_DIR` (default `./uploads`). Per-organization storage is capped under a MySQL advisory lock. Orphans older than a week that no event, organization, host or sponsor references are swept from the job loop. Keep generated filenames random and the filename regex strict.
...
The REST API currently covers public event search and authenticated event list/get/create; the remaining resources expected by the MCP server, SDK generation, and outbound webhooks are not implemented yet. Other gaps: check-in scanner, uploads for org logos/host avatars/sponsor logos and S3 storage, waitlist, discount codes UI, Stripe Connect onboarding for Cloud, private-event invitations, and an org switcher beyond the cookie default. Event slugs are still globally unique even though pages live under `/{org}/{event}`. Test data: `pnpm db:seed` creates the demo org but no user; sign in with any email and create your own org. See the README status list before adding anything, and update it when you ship a piece.
