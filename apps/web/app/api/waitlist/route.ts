...
import { z } from "zod";
import { joinWaitlist, waitlistJoinInput } from "@ot/core/services";
...
const input = waitlistJoinInput.extend({ eventId: z.string().length(26) });

/** Join the waitlist of a sold-out event. Idempotent per email. */
export async function POST(request: Request) {
  const parsed = input.safeParse(await readJsonBody(request, 4_096).catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "Enter your name and a valid email." }, { status: 400 });
  const { eventId, ...form } = parsed.data;
