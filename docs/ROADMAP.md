...
- [x] Launch hardening (2026-09-11): optional Turnstile / reCAPTCHA v3 check on sign-in links, registrations, waitlist joins and abuse reports (`CAPTCHA_*`), a top-edge navigation progress bar, and a single-VM production stack (`deploy/`: Caddy with automatic HTTPS, app image, MySQL, nightly backup script).
