...
export function WaitlistJoin({ eventId, eventName }: { eventId: string; eventName: string }) {
  ...
  if (done) return <p aria-live="polite" className="mt-4 text-sm">{done}</p>;
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <p className="mt-3 text-sm text-muted-foreground">Sold out. Join the waitlist and we&rsquo;ll email you if a spot opens up.</p>
      <DialogTrigger asChild><Button variant="event" size="lg" className="mt-3 w-full">Join the waitlist</Button></DialogTrigger>
      <DialogContent>
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          <div><Label htmlFor="wl-name">Name</Label><Input id="wl-name" required value={name} onChange={(e) => setName(e.target.value)} className="mt-1.5" /></div>
          <div><Label htmlFor="wl-email">Email</Label><Input id="wl-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1.5" /></div>
          {error && <p role="alert" className="text-sm text-destructive">{error}</p>}
          <Button type="submit" variant="event" className="w-full" disabled={busy}>{busy ? "Joining…" : "Join the waitlist"}</Button>
        </form>
      </DialogContent>
    </Dialog>
  );
