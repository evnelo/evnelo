...
- Inventory: `ticket_types.sold` and `held`. Free orders increment `sold` immediately; paid orders increment `held` for a 10-minute hold and move to `sold` only when Stripe settles before the deadline or from the durable `processing` state. Reservation is a conditional UPDATE, not a SELECT then INSERT. Before a pending hold is released, `expireHolds` cancels its PaymentIntent; a late successful payment is marked expired and refunded idempotently. Verified delayed methods move the order to `processing`, clear the short deadline, retain inventory, and resolve through Stripe webhooks or resume polling.
- Paid checkout: `RegisterForm` creates the held order, then `RegisterCard` switches to `PaymentStep`, which mounts Stripe's Payment Element with the returned client secret. Never show registration success merely because an order exists: classify the PaymentIntent result with `lib/payment-flow.ts`. Redirect payment methods return with a signed resume token; credentials are copied to per-event `sessionStorage` and removed from the URL immediately under `Referrer-Policy: no-referrer`. `/api/orders/resume` binds them to the displayed event, database order, stored PaymentIntent/client secret, and server-selected connected account before restoring state.
...
- Event cover/logo uploads are authenticated, use DB-backed per-user rate limits, and fail closed on request origin. They are limited to 5 MB JPEG/PNG/WebP, decoded with pixel limits and re-encoded as WebP, stored in the absolute persistent `UPLOAD_DIR`, and served publicly through `/api/uploads/{filename}` with immutable caching. Per-organization storage is capped under a MySQL advisory lock and stale unreferenced uploads are swept after 24 hours. Keep generated filenames random and path validation strict.
...
- REST API: implemented endpoints are documented at `/api/v1/openapi.json`. Organization routes authenticate SHA-256-hashed keys, enforce read/write scopes and a 120-request/minute per-key limit, return rate-limit headers after quota consumption, and cap JSON bodies at 256 KiB. Public discovery uses MySQL full-text indexes plus per-client/global throttles. Forwarded IP headers are ignored unless `API_TRUSTED_PROXY_HEADER` selects the single header overwritten by a trusted proxy. Event creation is always transactional; using `Idempotency-Key` also claims and completes the key in the operation transaction, and expired keys may be reused after 24 hours.
